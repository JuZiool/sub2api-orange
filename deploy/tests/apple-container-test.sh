...
[[ ! -s "${STATE_DIR}/network-subnets/sub2api-apple" ]] ||     fail "up passed a subnet when APPLE_CONTAINER_NETWORK_SUBNET was unset"
...
' >>"${ENV_FILE}"
"${SCRIPT}" up
[[ "$(<"${STATE_DIR}/network-subnets/sub2api-apple")" == "172.31.250.0/24" ]] ||     fail "up did not pass APPLE_CONTAINER_NETWORK_SUBNET to network creation"
...
[[ "${mismatch_output}" == *"Existing network 'sub2api-apple' uses subnet '172.31.250.0/24'"* ]] ||     fail "up did not explain the existing network subnet mismatch"
[[ "${mismatch_output}" == *"destroy --yes"* ]] ||     fail "up did not provide the network migration command"
