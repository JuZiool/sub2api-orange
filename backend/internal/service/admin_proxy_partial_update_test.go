...
func TestAdminProxyPartialUpdateClearsAndSetsSettings(t *testing.T) {
	...
	got, err := svc.UpdateProxy(context.Background(), 9, &UpdateProxyInput{
		ClearExpiresAt: true, FallbackMode: FallbackModeNone, ClearBackupID: true, ExpiryWarnDays: 0, ExpiryWarnDaysSet: true,
	})
	require.NoError(t, err)
	...
	days := 3
	got, err = svc.UpdateProxy(context.Background(), 9, &UpdateProxyInput{
		ExpiresAt: &expiry, FallbackMode: FallbackModeProxy, BackupProxyID: &backup, ExpiryWarnDays: days, ExpiryWarnDaysSet: true,
	})
	require.NoError(t, err)
...
func TestAdminProxyPartialUpdateValidatesMergedFallback(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "negative warning", input: UpdateProxyInput{ExpiryWarnDays: negative, ExpiryWarnDaysSet: true}, wantError: true},
	} {
