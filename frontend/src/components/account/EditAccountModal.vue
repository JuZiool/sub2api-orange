<template>
  ...
  >
    ...
    >
      ...
      <!-- Orange 特有：Codex 额度透支账号级关闭开关（仅 OpenAI OAuth） -->
      <div
      ...
      >
        <div class="flex items-center justify-between gap-4">
          ...
          <button
            type="button"
            data-testid="edit-codex-quota-overdraft-disabled-toggle"
            role="switch"
...
// Orange 特有：Codex 额度透支账号级关闭开关（仅 OpenAI OAuth），缺省继承全局
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'setup-token' || newAccount.type === 'apikey')) {
    ...
    codexQuotaOverdraftDisabled.value =
      newAccount.type === 'oauth' && extra?.codex_quota_overdraft_disabled === true
  ...
  grokMediaEligibilityMode.value = modeFromGrokMediaExtra(newAccount.extra)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'setup-token' || props.account.type === 'apikey')) {
      ...
      // 缺省即允许透支；仅显式关闭时写入，避免 extra 堆积默认项
      if (props.account.type === 'oauth' && codexQuotaOverdraftDisabled.value) {
        newExtra.codex_quota_overdraft_disabled = true
      } else {
        delete newExtra.codex_quota_overdraft_disabled
      }
