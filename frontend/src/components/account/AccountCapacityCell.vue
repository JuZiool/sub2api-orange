<template>
  <div class="flex flex-col gap-0.5">
    <!-- 并发槽位（多代理池显示总计及各代理占用，Orange 特有） -->
    <template v-if="hasProxyPoolUsage">
      <CapacityBadge
        :color-class="totalConcurrencyClass"
        :tooltip="totalConcurrencyTooltip"
        :current="totalCurrentConcurrency"
      ...
      <CapacityBadge v-for="proxy in account.proxy_pool" :key="proxy.proxy_id"
        ...
        :tooltip="proxy.proxy_name"
        :current="proxy.current_concurrency" :max="proxy.max_concurrency">
        <svg class="h-2.5 w-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75A2.25 2.25 0 0115.75 13.5H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z" />
        </svg>
      </CapacityBadge>
...
const hasMultipleProxies = computed(() => (props.account.proxy_ids?.length ?? 0) > 1)
const hasProxyPoolUsage = computed(() => hasMultipleProxies.value && proxyPool.value.length > 0)
const hasCompleteProxyPoolUsage = computed(() =>
  hasProxyPoolUsage.value && proxyPool.value.every(proxy =>
    Number.isFinite(proxy.current_concurrency) && Number.isFinite(proxy.max_concurrency)
  )
)

const totalCurrentConcurrency = computed(() => {
  if (!hasCompleteProxyPoolUsage.value) return currentConcurrency.value
  return proxyPool.value.reduce((total, proxy) => total + proxy.current_concurrency, 0)
})

const totalMaxConcurrency = computed(() => {
  if (hasCompleteProxyPoolUsage.value) {
    return proxyPool.value.reduce((total, proxy) => total + proxy.max_concurrency, 0)
  }
  return props.account.concurrency * (props.account.proxy_ids?.length ?? proxyPool.value.length)
})
...
const totalConcurrencyClass = computed(() =>
  concurrencyClassFor(totalCurrentConcurrency.value, totalMaxConcurrency.value)
)

const totalConcurrencyTooltip = computed(() =>
  t('admin.accounts.capacity.proxyPoolTotal', { count: proxyPool.value.length })
)

const concurrencyClass = computed(() => concurrencyClassFor(currentConcurrency.value, props.account.concurrency))

const proxyConcurrencyClass = (current: number, max: number) => concurrencyClassFor(current, max)
