export default {
    accounts: {
      ...
      openai: {
        ...
        codexFingerprintModeDesc: '默认关闭。保留官方设备、会话和完整收敛模式；单机多窗口额外提供稳定窗口身份与 Mac TLS 配置，保留独立对话缓存，不调整并发或隔离账号。',
        codexFingerprintOff: '关闭（透传，默认）',
        codexFingerprintSingleMachineMultiWindow: '单机多窗口（对话+子智能体）',
        codexFingerprintDevice: '仅设备',
        codexFingerprintSession: '设备+会话',
        codexFingerprintFull: '完全收敛',
        codexTurnTicket: 'Codex {length} 门票',
        codexTurnTicketDesc: '显示已配置模型的门票状态。仅在启用缺票拦截时，没有有效门票的模型才会暂停调度。',
        codexTurnTicketMissing: '暂无有效 {length} 门票，仍允许请求',
        codexTicketAccountEnabled: 'Codex 292 / 332 打票',
        codexTicketAccountEnabledDesc: '使用网关统一配置的打票代理池。按账号订阅自动识别：Pro 为 292，Team 为 332；只对当前 OpenAI OAuth / Setup Token 账号生效。',
        codexTicketFailClosed: '缺票时禁用模型',
        codexTicketFailClosedDesc: '默认开启。没有有效门票时暂停对应模型；关闭后仍会打票，但缺票时继续放行请求。',
        codexTurnTicketReady: '剩余 {time}',
        codexTurnTicketPaused: '未打到 {length}，该模型已暂停',
        codexTicketDiagnostics: {
          inProgress: '正在打票',
          paused: '自动重试已暂停',
          lastError: '最近失败：{reason}',
          attempts: '已尝试 {count} 次',
          successCount: '累计成功：{count} 次',
          failureCount: '累计失败：{count} 次',
          injectMissCount: '注入缺失：{count} 次',
          lastInjectMiss: '最近注入缺失：{time}',
          compactCounters: '成功 {success} / 失败 {failure} / 缺失 {missed}',
          counterHint: '统计仅记录本次服务运行，重启或停用打票后清空。注入缺失只统计进入出站注入时缺票，不含调度阶段因缺票跳过的账号。',
          failures: '连续失败：{count} 次',
          length: '返回 / 目标长度：{actual} / {target}',
          httpStatus: '上游状态码：{status}',
          proxy: '最近使用：代理 #{index}',
          lastAttempt: '最近尝试：{time}',
          nextRetry: '下次重试：{time}',
          nextCheck: '恢复检查时间：{time}',
          nextRefresh: '下次续票：{time}',
          unknownPlan: '未识别账号套餐，暂按 {length} 判断；请检查套餐信息，Team 应为 332。',
          errors: {
            auth: '账号认证失败，请检查或刷新认证',
            forbidden: '上游拒绝访问，请检查账号权限',
            model_unsupported: '账号没有该模型的使用权限',
            quota: '上游额度不足',
            rate_limited: '上游限流，等待后重试',
            proxy_auth: '代理认证失败，请检查代理配置',
            network: '代理或上游连接失败',
            timeout: '上游请求超时',
            upstream_5xx: '上游服务暂时异常',
            invalid_response: '上游响应格式异常',
            length_mismatch: '返回门票长度与账号套餐不符',
            invalid_ticket: '未返回有效门票',
            no_proxy: '网关未配置打票代理',
            proxy_cooldown: '代理正在冷却，等待可用出口',
            canceled: '打票请求已取消',
            unknown: '打票失败，请检查服务日志',
          },
        },
        codexTicketHistory: {
          harvesting: '打票中',
          ...
          empty: '暂无采票记录',
          loadFailed: '采票历史加载失败，请刷新重试',
          success: '成功',
          failure: '失败',
          canceled: '已取消',
          length: '长度 {actual} / {target}',
          proxy: '代理 #{index}',
