export default {
    accounts: {
      ...
      openai: {
        ...
        codexFingerprintModeDesc: 'Off by default. Official device, session and full modes remain available. Single-machine mode adds stable windows and a Mac TLS profile, preserves conversation caches, and does not change concurrency or isolate accounts.',
        ...
        codexTurnTicket: 'Codex {length} ticket',
        ...
        codexTurnTicketMissing: 'No valid {length} ticket; requests remain allowed',
        codexTicketAccountEnabled: 'Codex 292 / 332 harvesting',
        codexTicketAccountEnabledDesc: 'Uses the gateway-configured harvest proxy pool. The length follows the subscription: 292 for Pro, 332 for Team. Applies only to OpenAI OAuth / Setup Token accounts.',
        codexTicketFailClosed: 'Disable models without a ticket',
        codexTicketFailClosedDesc: 'Enabled by default. Pauses a model without a valid ticket; when off, harvesting continues but requests are still allowed without a ticket.',
        codexTurnTicketReady: '{time} left',
        codexTurnTicketPaused: 'No {length} ticket; this model is paused',
        codexTicketDiagnostics: {
          inProgress: 'Harvesting ticket',
          paused: 'Automatic retries paused',
          lastError: 'Last failure: {reason}',
          attempts: '{count} attempts',
          successCount: 'Total successes: {count}',
          failureCount: 'Total failures: {count}',
          injectMissCount: 'Injection misses: {count}',
          lastInjectMiss: 'Last injection miss: {time}',
          compactCounters: 'OK {success} / Failed {failure} / Missed {missed}',
          counterHint: 'Counters cover this service run and clear on restart or when tickets are disabled. Injection misses count missing tickets at outbound injection, excluding accounts skipped during scheduling.',
          failures: 'Consecutive failures: {count}',
          length: 'Returned / target length: {actual} / {target}',
          httpStatus: 'Upstream status: {status}',
          proxy: 'Last used: proxy #{index}',
          lastAttempt: 'Last attempt: {time}',
          nextRetry: 'Next retry: {time}',
          nextCheck: 'Next recovery check: {time}',
          nextRefresh: 'Next renewal: {time}',
          unknownPlan: 'Subscription not recognized; using {length}. Check the account plan; Team requires 332.',
          errors: {
            auth: 'Account authentication failed; check or refresh credentials',
            forbidden: 'Upstream denied access; check account permissions',
            model_unsupported: 'This account cannot access the model',
            quota: 'Upstream quota exhausted',
            rate_limited: 'Upstream rate limited; waiting to retry',
            proxy_auth: 'Proxy authentication failed; check proxy settings',
            network: 'Could not connect to proxy or upstream',
            timeout: 'Upstream request timed out',
            upstream_5xx: 'Upstream service is temporarily unavailable',
            invalid_response: 'Unexpected upstream response format',
            length_mismatch: 'Returned ticket length does not match the account plan',
            invalid_ticket: 'No valid ticket returned',
            no_proxy: 'No harvest proxy configured in gateway settings',
            proxy_cooldown: 'Proxies are cooling down; waiting for an available route',
            canceled: 'Ticket request canceled',
            unknown: 'Ticket harvesting failed; check service logs',
          },
        },
        codexTicketHistory: {
          ...
          loadFailed: 'Could not load harvest history; refresh and retry',
          success: 'Success',
          failure: 'Failure',
          canceled: 'Canceled',
      ...
      usageWindow: {
        ...
        estimatedTotalCostTooltip: 'Estimated total cost at 100% utilization based on the current window cost and usage.',
      },
      openaiQuotaReset: {
        count: 'Credits',
        reset: 'Reset',
