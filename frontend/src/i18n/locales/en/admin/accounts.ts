export default {
    accounts: {
      ...
      openai: {
        ...
        codexFingerprintModeDesc: 'Keep Codex identity signals stable and isolated per account to reduce upstream-visible device and session count. This changes identity/cache/session markers, not the proxy IP or network egress. Accounts without an explicit mode default to the single-machine multi-window mode; some accounts reported quota shrinkage after enabling convergence, so choose based on your own measurements.',
        codexFingerprintOff: 'Off (passthrough)',
        codexFingerprintAccountDevice: 'Unique device per account (new, recommended)',
